...
MZ_BREAKPOINTS = dict(cascade_config['materialize']['breakpoints'])
MZ_BREAKPOINT_KEYS = list(tp[0] for tp in cascade_config['materialize']['breakpoints'])
    ...
    def render(self):
        return format_html('<div class="form-row">{0}</div>',
            format_html_join('', '<div class="field-box">'
                ...
                '</div>', ((force_text(w), MZ_BREAKPOINTS[w.choice_value][1]) for w in self)
            ))


class MaterializeContainerForm(ModelForm):
    """
...
class MaterializeContainerPlugin(MaterializePluginBase):
    name = _("Container")
    require_parent = False
    form = MaterializeContainerForm
    WIDGET_CHOICES = (
        ('s', _("Tiny (<{s[0]}px)".format(**MZ_BREAKPOINTS))),
        ('m', _("Small (≥{m[0]}px and <{l[0]}px)".format(**MZ_BREAKPOINTS))),
        ('l', _("Large (≥{l[0]}px)".format(**MZ_BREAKPOINTS))),
    )
    glossary_fields = (
        PartialFormField('breakpoints',
            ...
            initial=['l', 'm', 's'],
            help_text=_("Supported display widths for Materialize's grid system.")
        ),
    )
    ...
    def get_identifier(cls, obj):
        identifier = super(MaterializeContainerPlugin, cls).get_identifier(obj)
        breakpoints = obj.glossary.get('breakpoints')
        content = obj.glossary.get('fluid') and '(fluid) ' or ''
        if breakpoints:
            devices = ', '.join([force_text(MZ_BREAKPOINTS[bp][2]) for bp in breakpoints])
            content = _("{0}for {1}").format(content, devices)
        return format_html('{0}{1}', identifier, content)

    @classmethod
    def get_css_classes(cls, obj):
        css_classes = super(MaterializeContainerPlugin, cls).get_css_classes(obj)
    ...
    def save_model(self, request, obj, form, change):
        super(MaterializeContainerPlugin, self).save_model(request, obj, form, change)
        obj.sanitize_children()

    @classmethod
    def sanitize_model(cls, obj):
        sanitized = super(MaterializeContainerPlugin, cls).sanitize_model(obj)
        ...
        for index, bp in enumerate(breakpoints):
            try:
                ...
                max_widths[bp] = MZ_BREAKPOINTS[bp][3]
            if last_index > 0:
                if index == 0:
                    next_bp = breakpoints[1]
                    media_queries[bp] = ['(max-width: {0}px)'.format(MZ_BREAKPOINTS[next_bp][0])]
                elif index == last_index:
                    media_queries[bp] = ['(min-width: {0}px)'.format(MZ_BREAKPOINTS[bp][0])]
                else:
                    next_bp = breakpoints[index + 1]
                    media_queries[bp] = ['(min-width: {0}px)'.format(MZ_BREAKPOINTS[bp][0]),
                                         '(max-width: {0}px)'.format(MZ_BREAKPOINTS[next_bp][0])]
        return sanitized
    ...
    def get_parent_classes(self, slot, page):
        if self.cms_plugin_instance and self.cms_plugin_instance.parent:
            ...
            return []
        return super(MaterializeContainerPlugin, self).get_parent_classes(slot, page)

plugin_pool.register_plugin(MaterializeContainerPlugin)


class MaterializeRowForm(ManageChildrenFormMixin, ModelForm):
    """
...
class MaterializeRowPlugin(MaterializePluginBase):
    name = _("Row")
    default_css_class = 'row'
    parent_classes = ('MaterializeContainerPlugin', 'MaterializeColumnPlugin',)
    form = MaterializeRowForm
    fields = ('num_children', 'glossary',)

    @classmethod
    def get_identifier(cls, obj):
        identifier = super(MaterializeRowPlugin, cls).get_identifier(obj)
    ...
    def save_model(self, request, obj, form, change):
        ...
        super(MaterializeRowPlugin, self).save_model(request, obj, form, change)
        ...
        self.extend_children(obj, wanted_children, MaterializeColumnPlugin, child_glossary=child_glossary)

plugin_pool.register_plugin(MaterializeRowPlugin)


class MaterializeColumnPlugin(MaterializePluginBase):
    name = _("Column")
    parent_classes = ('MaterializeRowPlugin',)
    ...
    default_css_attributes = list(itertools.chain(*(('{}-column-width'.format(s),
        ...
        for s in MZ_BREAKPOINT_KEYS)))
    glossary_variables = ['container_max_widths']

    def get_form(self, request, obj=None, **kwargs):
        def chose_help_text(*phrases):
            if next_bp:
                return phrases[0].format(*MZ_BREAKPOINTS[next_bp])
            elif len(breakpoints) > 1:
                return phrases[1].format(*MZ_BREAKPOINTS[bp])
            else:
                return phrases[2]
        ...
        if isinstance(parent_plugin, MaterializePluginBase):
            breakpoints = parent_obj.get_complete_glossary()['breakpoints']
            for bp in breakpoints:
                try:
                    next_bp = breakpoints[breakpoints.index(bp) + 1]
                    last = MZ_BREAKPOINT_KEYS.index(next_bp)
                except IndexError:
                    next_bp = None
                    last = None
                finally:
                    first = MZ_BREAKPOINT_KEYS.index(bp)
                    devices = ', '.join([force_text(MZ_BREAKPOINTS[b][2]) for b in MZ_BREAKPOINT_KEYS[first:last]])
        ...
        return super(MaterializeColumnPlugin, self).get_form(request, obj, **kwargs)

    def save_model(self, request, obj, form, change):
        super(MaterializeColumnPlugin, self).save_model(request, obj, form, change)
        obj.sanitize_children()

    @classmethod
    def sanitize_model(cls, obj):
        sanitized = super(MaterializeColumnPlugin, cls).sanitize_model(obj)
        parent_glossary = obj.get_parent_glossary()
        column_units = 12
        obj.glossary['container_max_widths'] = {}
        breakpoints = parent_glossary.get('breakpoints', [])
        for bp in MZ_BREAKPOINT_KEYS:
            ...
            if bp in breakpoints:
                ...
                new_width = parent_glossary['container_max_widths'][bp] * column_units / 12 - cascade_config['materialize']['gutter']
    ...
    @classmethod
    def get_identifier(cls, obj):
        identifier = super(MaterializeColumnPlugin, cls).get_identifier(obj)
...
plugin_pool.register_plugin(MaterializeColumnPlugin)